...
export const opticalLayouts = {
  ...
  ramanIntro: {
    ...
    beams: [],
    info: {
      purpose: "显微共焦拉曼光谱 (Raman) 的核心是测量光子与晶格振动之间的非弹性散射。谱线的频移对应声子能量，峰宽和强度进一步反映应力、层数、缺陷、温度以及晶体取向。",
      polarization: "偏振 Raman 不是简单改变亮度，而是改变入射偏振 e_i 和散射偏振 e_s 与 Raman 张量 R 的投影关系，用于区分声子对称性和晶轴方向。",
      notes: [
        "Raman 峰的横轴通常是相对激发光的频移 cm^-1，而不是发射波长本身。",
        "Stokes 峰表示散射光少了一个声子能量；Anti-Stokes 峰表示散射光多了一个声子能量，强度常受温度影响。",
        "Notch 滤片负责去除强瑞利光；1800 g/mm 光栅用于解析窄峰和小频移差异。"
      ]
    }
  },
  ...
  plIntro: {
    ...
    beams: [],
    info: {
      purpose: "光致发光 (PL) 测量样品吸收激发光后自发辐射出的光。它主要反映电子-空穴、激子、带电激子和缺陷态的辐射复合过程，适合读带隙、发光效率和局域态。",
      polarization: "线偏振 PL 读发光偶极的方向性和低对称晶体各向异性；圆偏振 PL 读 TMDs 的 K/K' 谷选择定则、谷极化保持和磁场诱导的 Zeeman 响应。",
      notes: [
        "PL 的横轴通常是发射能量或波长；主峰位置对应辐射跃迁能量，不等同于激发光能量。",
        "激发光需要满足 hν_exc > Eg，载流子先弛豫到低能态，再以激子、Trion 或缺陷态复合发光。",
        "150 g/mm 光栅牺牲部分分辨率换取宽谱覆盖，适合一次记录宽 PL 包络和多个发光通道。"
      ]
    }
  },
  ...
  plPolarized: {
    ...
    info: {
      ...
      notes: [
        ...
        "常用于测量二维各向异性半导体（如 ReS₂、黑磷 BP）的偏振依赖激子发射。",
  ...
  plValley: {
    ...
    info: {
      ...
      notes: [
        ...
        "样品放置于 1.7K 低温、强垂直超导磁场 (B_z) 环境中，以诱导 Zeeman 分裂并增强自旋谷寿命。",
  ...
  lifetime: {
    ...
    info: {
      purpose: "时间分辨发光光谱 (TRPL) 不只看发光强不强，而是看发光随时间如何衰减。脉冲激光给出时间零点，SPAD 与 TCSPC 统计单光子到达时间，从而得到复合寿命 τ。",
      polarization: "该模块默认读积分发光寿命；若加入偏振选择，τ 的变化可进一步区分不同发光偶极、暗态转换或谷退极化通道。",
      notes: [
        "单指数衰减常对应一个主导复合通道；多指数衰减通常意味着缺陷、暗态、能量转移或多种寿命并存。",
        "SPAD 提供单光子探测，TCSPC 将每个光子的到达时间累积成直方图，再拟合 I(t)=I0 exp(-t/τ)。",
        "实际寿命分析需要考虑仪器响应函数 IRF、背景扣除和早期时间窗的卷积效应。"
      ]
    }
  },
  ...
  shgIntro: {
    ...
    beams: [],
    info: {
      purpose: "二次谐波发生 (SHG) 测的是材料的二阶非线性极化响应。强基频光场在非中心对称材料中诱导 P(2ω)，辐射出波长减半、频率加倍的相干光。",
      polarization: "极角分辨 SHG 通过旋转入射和检出偏振来读 χ(2) 张量。单层 D3h TMD 常出现六瓣极图，可用于判断晶轴方向与反演对称性破缺。",
      notes: [
        "SHG 是相干虚能级过程，不是 PL 的真实载流子复合，也不是 Raman 的频移散射。",
        "1064 nm 基频光在样品处产生 532 nm 二倍频光；滤片必须彻底去除残余 1064 nm。",
        "电偶极 SHG 对反演对称性非常敏感：理想中心对称材料的体相 χ(2) 近似为零。"
      ]
    }
  },
